...
namespace DoomLauncher
{
    public partial class MainForm
    {
            ...
            PlayOptions playOptions = PlayOptions.None)
        {
            ...
            if (m_launchArgs.LaunchSourcePortID.HasValue)
            {
                m_currentPlayForm.SelectedSourcePort =
                    DataSourceAdapter.GetSourcePort(m_launchArgs.LaunchSourcePortID.Value);
            }
            if (m_launchArgs.LaunchIWadID.HasValue)
            {
                m_currentPlayForm.SelectedIWad = DataSourceAdapter.GetGameFileIWads()
                    .FirstOrDefault(x => x.IWadID == m_launchArgs.LaunchIWadID.Value);
            }
            if (map != null)
        ...
        {
            if (m_launchArgs.LaunchDefaultProfile)
                return (GameFile)gameFile;

            int? requestedProfileId = m_launchArgs.LaunchGameProfileID
                ?? gameFile.SettingsGameProfileID;
            if (requestedProfileId.HasValue)
            {
                var profile = DataSourceAdapter.GetGameProfiles(gameFile.GameFileID.Value)
                    .FirstOrDefault(x => x.GameProfileID == requestedProfileId.Value);
                if (profile != null)
                    return profile;
