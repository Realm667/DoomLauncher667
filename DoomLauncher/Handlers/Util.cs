    ...
    public static class Util
    {
        ...
        private static void DisplayException(Form form, Exception ex)
        {
            ...
            else
            {
                ...
                {
                    // Clipboard access can be unavailable in remote or sandboxed sessions.
                }
        ...
        public static void LogUnexpectedException(Exception ex)
        {
            try
            {
                string logPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "DoomLauncher.error.log");
                string entry = $"[{DateTimeOffset.Now:O}]{Environment.NewLine}{ex}{Environment.NewLine}{Environment.NewLine}";
                File.AppendAllText(logPath, entry);
            }
            catch
            {
                // Error reporting must never mask the original exception.
            }
        }
